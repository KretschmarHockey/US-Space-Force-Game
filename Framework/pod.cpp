...
void
Pod::Process(float deltaTime)
{

	m_previousX = m_x;
	m_previousY = m_y;

	int speed = Game::GetInstance().GetIniParser()->GetValueAsInt("pod", "speed");
	int range = Game::GetInstance().GetIniParser()->GetValueAsInt("pod", "speedRange");

	if (!xMoving && !yMoving)
	{
		endPosX = static_cast <float> (rand() % (Game::GetWidth() - GetSprite()->GetWidth()));
		endPosY = static_cast <float> (rand() % (Game::GetHeight() - 2 * GetSprite()->GetHeight()));
